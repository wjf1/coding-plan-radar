...
// ===== 本站变更记录 =====
const CHANGELOG = [
  {date:"2026-09-23", v:"v6.1", text:"核查修复：① 修复 record-history.mjs 的解析方式——原实现用 JSON.parse 解析键名不带引号的 PLAN_DATA 字面量必抛异常，而该步骤在 CI 中没有容错，会导致每日巡检整体失败，现改走 data.js 的 _CP_EXPORT 扩展点；② 修复语言切换按钮未绑定点击事件（setLang 从未被调用，点「中/EN」无反应），并补上按钮高亮态同步与 ?lang=en 首屏生效；③ 补齐英文态静态标签——章节标题、对比表与 Token 榜表头、工具条、排序选项、计算器标签、搜索占位符接入 I18N 字典（新增 data-i18n 通用机制，中文态还原原始标记）；④ 页头「数据更新」与页脚日期不再硬编码 2026-09-13，统一改读 data/meta.json。"},
  {date:"2026-09-23", v:"v6", text:"双轮迭代（31 项）：① 平台扩展至 19 家在售平台——新增 DeepSeek / 百度千帆 / 讯飞星辰 / 腾讯云 / Gemini，以及 TRAE / Devin / Poe（另有 1 家已停售平台保留为历史记录，共 20 个条目、69 个付费档位）；② 新增价格历史趋势——每日起步价快照保留 90 天，页面内纯 SVG 迷你图呈现走势，涨价/降价自动告警；③ 新增多选对比视图（勾选 2-4 个平台并排比较）、中英双语切换、URL 状态持久化（筛选/排序/搜索/对比项/语言随链接复现）、CSV 导出；④ 可访问性——语义化 ARIA 标签、aria-pressed 与选中态同步、键盘可达的「跳到主内容」链接、移动端触摸滚动；⑤ 数据真实性——消除 CI 故障静默、恢复 claude/cursor 监控、修复 livePrice 降级、识别反爬壳页面、扩展 issue 告警条件、降低噪声源线索占比；⑥ 质检修复——esc() 安全转义、CSV 换行处理、Hero 区国际化、对比项 URL 上限截断、record-history 解析安全化、首屏闪烁消除、90 天裁剪边界修正。"},
